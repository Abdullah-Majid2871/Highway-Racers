	...
	public void draw(Graphics g) {
		...
		if (GamePanel.p1VisionBlock) {
			for (int i = 0; i < 50; i++) {
				...
				g.drawImage(singleLeafImage, GamePanel.rando.nextInt(0,(int)(GamePanel.GAME_WIDTH/2 - GamePanel.GAME_WIDTH/48 - LEAVES_WIDTH)), (int)GamePanel.rando.nextInt(0, GamePanel.GAME_HEIGHT- LEAVES_HEIGHT), LEAVES_WIDTH, LEAVES_HEIGHT,null);
		...
		if (GamePanel.p2VisionBlock) {
			for (int i = 0; i < 50; i++) {
				...
				g.drawImage(singleLeafImage, GamePanel.rando.nextInt((GamePanel.GAME_WIDTH /2+ GamePanel.GAME_WIDTH/48), GamePanel.GAME_WIDTH - LEAVES_WIDTH), GamePanel.rando.nextInt(0, GamePanel.GAME_HEIGHT - LEAVES_HEIGHT), LEAVES_WIDTH, LEAVES_HEIGHT,null);
